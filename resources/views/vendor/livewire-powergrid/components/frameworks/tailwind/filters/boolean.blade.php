...
<div>
    @if (filled($booleanFilter))
        <div @class([
            ...
            @if (!$inline)
                <label class="text-gray-700 dark:text-gray-300" for="input_boolean_filter_{{ data_get($booleanFilter, 'field') }}">
                    {{ data_get($booleanFilter, 'label') }}
                </label>
            @endif
            <div class="relative">
                <select id="input_boolean_filter_{{ data_get($booleanFilter, 'field') }}" class="power_grid {{ $theme->selectClass }} {{ $class }} {{ data_get($column, 'headerClass') }}" style="{{ data_get($column, 'headerStyle') }}" wire:input.lazy="filterBoolean('{{ $booleanFilter['dataField'] }}', $event.target.value, '{{ $booleanFilter['label'] }}')" wire:model="filters.boolean.{{ $booleanFilter['dataField'] }}">
